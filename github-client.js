...
async function saveMenuToRepo(menuData) {
  const token = getGitHubToken();
  if (!token) throw new Error("Connect your GitHub token above before saving.");
...
async function uploadMenuImage(filename, base64Content) {
  const token = getGitHubToken();
  if (!token) throw new Error("Connect your GitHub token above before uploading photos.");
